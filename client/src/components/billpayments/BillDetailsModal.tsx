    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div className={`w-full max-w-2xl max-h-[90vh] rounded-lg shadow-lg flex flex-col ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
                {/* Modal Header */}
                <div className={`flex items-center justify-between px-6 py-4 border-b flex-shrink-0 ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                    <div>
                        <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Bill Details</h3>
                        <p className={`text-sm mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Dispense Code: <span className="font-mono font-medium">{dispenseCode}</span></p>
                    </div>
                ...
                <div className={`px-6 py-4 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
                    <div className="flex justify-between items-center">
                        <div className="space-y-1">
                            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Employee: <span className={isDarkMode ? 'text-white' : 'text-gray-900'}>{employeeName}</span></p>
                            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Time: <span className={isDarkMode ? 'text-white' : 'text-gray-900'}>{timestamp}</span></p>
                        </div>
                        ...
                        <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-100'}`}>
                            {items.map((item) => (
                                <tr key={item.id} className="text-sm">
                                    <td className={`py-3 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{item.name}</td>
                                    <td className={`py-3 text-center ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{item.quantity}</td>
                                    <td className={`py-3 text-right ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>₦{item.pricePerUnit.toLocaleString()}</td>
                                    <td className={`py-3 text-right ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>₦{item.totalPrice.toLocaleString()}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
