...
function ForgotPassword() {
    ...
    return (
        <div className={`w-full min-h-dvh flex justify-center items-center ${isDarkMode ? 'gradient-to-br from-gray-900 via-gray-800 to-gray-900' : 'gradient-to-br from-slate-900 via-emerald-900 to-slate-900'}`}>
            <div className={`flex w-[400px] min-h-[450px] rounded-3xl shadow-2xl flex-col max-sm:w-full max-sm:min-h-[80dvh] max-sm:rounded-none max-sm:absolute max-sm:bottom-0 max-sm:rounded-t-3xl ${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white'}`}>
                <div className="flex flex-col justify-center items-center w-full pt-10 max-sm:pt-20">
                    <img src="/images/logo.png" alt="Logo" className="w-20"/>
                    <h1 className={`text-2xl font-bold mt-3 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
                        Forgot Password?
                    </h1>
                    <p className={`mt-2 text-center px-8 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                        {submitted 
                ...
                {submitted ? (
                    ...
                    <form className="flex flex-col w-full px-10 mt-8" onSubmit={handleSubmit}>
                        <input
                            type="email"
                            value={email}
                ...
                <div className="flex justify-center items-center w-full py-3 mt-4">
                    <p className={isDarkMode ? 'text-gray-400' : 'text-gray-600'}>
                        Remember your password? <Link to="/login" className="text-green-400 hover:text-green-500 hover:underline font-medium">Log in here</Link>
                    </p>
                </div>
            </div>
        </div>
